...
function pubEvent(e) {
  return { id: e.id, slug: e.slug, model: e.model, title: e.title, names: e.names, eyebrow: e.eyebrow,
    ...
    iban: e.iban || '', ibanLabel: e.ibanLabel || '', presentes: e.presentes || null, videoUrl: e.videoUrl || '',
...
app.get('/api/health', (req, res) => res.json({ ok: true, service: 'convitta-api', v: 8 }));
...
// Video upload (organizer) — for video invitations
const videoMw = multer({
  storage: multer.diskStorage({
    destination: (q, f, cb) => cb(null, UPLOADS),
    filename: (q, f, cb) => { const ext = (String(f.originalname).match(/\.[a-zA-Z0-9]+$/) || ['.mp4'])[0].toLowerCase(); cb(null, genId() + ext); }
  }),
  limits: { fileSize: 45 * 1024 * 1024 },
  fileFilter: (q, f, cb) => cb(null, /^video\//.test(f.mimetype))
});
app.post('/api/upload-video', orgAuth, videoMw.single('file'), (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'no_file' });
  res.json({ url: '/uploads/' + req.file.filename });
});
...
const EDITABLE = ['title','names','eyebrow','sub','date','time','place','whenISO','mapUrl','dressCode','giftUrl','initials','albumUrl','iban','ibanLabel','videoUrl'];
...
app.post('/api/events', orgAuth, async (req, res) => {
  ...
  await withLock(() => {
    ...
    events[id] = { id, orgId: req.orgId, slug, createdAt: Date.now(),
      ...
      videoUrl: String(b.videoUrl || '').slice(0, 500),
